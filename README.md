...
+ There are four primitive data types
    ...
    - `list`: `list` starts with `[` and ends with `]`. The elements in `list` are seperated by spaces, each of them is a part of this `list`. `list` can be nested. An example: `[1234.0 2 "Hello [true "false]]`

+ Although stored as `word`, value should be changed automatically if necessary. For example, the value of `or true "false` is `true`, and the value of `add "1.0 2` is `3.0`.
